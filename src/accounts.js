  ...
  try {
    ...
    if (error.name === 'maxBlocksTimeout') {
      ...
      if (!getNameAlreadyExists(oreAccountName)) {
        const err = new Error(`Error creating bridge account: ${oreAccountName}. Transaction hasn't been applied to chain`);
        err.name = error.name;
        throw err;
      }
    }
  ...
  try {
    ...
    if (error.name === 'maxBlocksTimeout') {
      ...
      if (!isPermissionOnChain) {
        const err = new Error(`Problem with adding permission: ${permissionName} with keys ${keys}`);
...
async function createBridgeAccount(password, salt, authorizingAccount, options) {
  ...
  if (!nameAlreadyExists || this.isNullOrEmpty(newAccountName)) {
    try {
      ...
      const awaitTransactionOptions = getAwaitTransactionOptions(options);
      try {
        transaction = await this.sendTransaction(async () => this.createNewAccount(authorizingAccount, keys, options), WaitForConfirm.Confirm_1, awaitTransactionOptions);
      } catch (error) {
        if (error.name === 'maxBlocksTimeout') {
          // Bad name for function
          if (!getNameAlreadyExists(oreAccountName)) {
