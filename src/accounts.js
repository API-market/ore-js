...
async function createOreAccountWithKeys(activePublicKey, ownerPublicKey, orePayerAccountName, options = {}) {
  ...
  const { transaction, confirmationStatus } = await this.sendTransaction(() => newAccountTransaction.bind(this)(oreAccountName, ownerPublicKey, activePublicKey, orePayerAccountName, options), confirm, awaitTransactionOptions);
  if (confirmationStatus === 'maxBlocksTimeout') {
    // Bad name for function
    if (!getNameAlreadyExists(oreAccountName)) {
      const error = new Error(`Error creating bridge account: ${oreAccountName}. Transaction hasn't been applied to chain`);
...
async function addPermission(authAccountName, keys, permissionName, parentPermission, overridePermission = false, options = {}) {
  ...
  const awaitTransactionOptions = getAwaitTransactionOptions(options);
  const { transaction, confirmationStatus } = await this.sendTransaction(async () => this.transact(actions, broadcast), confirm, awaitTransactionOptions);
  if (confirmationStatus === 'maxBlocksTimeout') {
    // Retrieve account permission from chain and compare to permissionName
    ...
    if (!isPermissionOnChain) {
      const error = new Error(`Problem with adding permission: ${permissionName} with keys ${keys}`);
      error.name = confirmationStatus;
      throw error;
    }
  }
...
async function exportAccount(authAccountName, publicKeys) {
  ...
  try {
    ...
    const { confirm = true } = options;
    const awaitTransactionOptions = getAwaitTransactionOptions(options);
    ({ transaction: activeTransaction } = await this.sendTransaction(() => addPermission.bind(this)(authAccountName, [active], 'active', 'owner', true, options), confirm, awaitTransactionOptions));
    ({ transaction: ownerTransaction } = await this.sendTransaction(() => addPermission.bind(this)(authAccountName, [owner], 'owner', '', true, options), confirm, awaitTransactionOptions));
  } catch (error) {
...
async function createBridgeAccount(password, salt, authorizingAccount, options) {
  ...
  if (!nameAlreadyExists || this.isNullOrEmpty(newAccountName)) {
    try {
      ...
      const awaitTransactionOptions = getAwaitTransactionOptions(options);
      ({ transaction, confirmationStatus } = await this.sendTransaction(async () => this.createNewAccount(authorizingAccount, keys, options), WaitForConfirm.Confirm_1, awaitTransactionOptions));
      if (confirmationStatus === 'maxBlocksTimeout') {
        // Bad name for function
        if (!getNameAlreadyExists(oreAccountName)) {
          throw new Error(`Error creating bridge account: ${oreAccountName}. Transaction hasn't been applied to chain`);
        }
      }
    } catch (error) {
